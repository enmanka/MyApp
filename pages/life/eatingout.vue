<template>
	<div class="container">
		...
		<div class="date-selector">
			<label for="date-picker">计划月份：</label>
			<input type="month" id="date-picker" v-model="currentDate" />
		</div>
...
export default {
	data() {
		return {
			...
			currentDate:  new Date().toISOString().slice(0, 7), // 当前日期
	...
	methods: {
		...
		cancelEntry() {
			...
			this.currentDate = new Date().toISOString().slice(0, 7);
		},
		...
		confirmEntry() {
						...
						// 与后端通信的代码，传入id(若id!=none则需要先删除对应月份的对应计划)、userId、月份等数据
						
						this.quantity = this.currentQuantity;
						
						uni.request({
							url: 'http://47.108.162.90:3000/diet/addPlan',
							method: 'POST',
							data: {
								userId: this.userId,
