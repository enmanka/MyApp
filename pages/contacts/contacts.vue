<template>
  <div class="contactsPage">
    ...
    <button class="add-button" @click="navigateTo('addContact')">
      <uni-icons type="plusempty" size="50" :style="{ color: selectedIcon === 'add' ? 'blue' : '#C7D4DD' }"></uni-icons>
    </button>
    ...
    <div v-for="(contactsGroup, letter) in groupedContacts" :key="letter" class="contacts-group">
      ...
      <div
        v-for="contact in contactsGroup"
        :key="contact.id"
        class="contact-item"
...
export default {
  ...
  methods: {
    ...
    navigateTo(page) {
      if (page === 'addContact') {
        uni.navigateTo({ url: '/pages/life/contacts/addContact' });
      }
    },
    // 跳转到联系人详情页
    navigateToContactDetails(contactId) {
      uni.navigateTo({ url: `/pages/life/contacts/showContact?id=${contactId}` });
    },
	//与后端连接的部分
    // 获取联系人数据
    async fetchContacts() {
      try {
        ...
        const response = await uni.request({
          ...
          method: 'GET',
          data: { userID: this.userID }, // 传递 userID
        });
